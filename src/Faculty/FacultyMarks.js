...
class FacultyMarks extends Component {
  ...
  filterMonths = (e) => {
    if (e.target.value !== "all") {
      ...
      const url = APIData.api + `marks/month?org=${org}&month=` + e.target.value;
  ...
  filterGrade = (e) => {
    if (e.target.value !== "all") {
      this.setState({ loading: true })
      const url = APIData.api + `marks/grade?org=${org}&grade=` + e.target.value;
      axios.get(url, { headers: APIData.headers })
        .then((response) => {
  ...
  filterType = (e) => {
    if (e.target.value !== "all") {
      this.setState({ loading: true })
      const url = APIData.api + `marks/assesment?org=${org}&type=` + e.target.value;
      axios.get(url, { headers: APIData.headers })
        .then((response) => {
          this.setState({ user: response.data, loading: false ,displayGraphButon:false,displayData:false});
        })
