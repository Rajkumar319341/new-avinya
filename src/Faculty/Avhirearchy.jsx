...
const Avhirearchy = ({ supervisorEmail }) => {
  ...
  const fetchData = (email) => {
    const url = APIData.api + "avinya/organization-hierarchy/supervisor?supervisorEmail=" + email;
    axios
  ...
  const fetchAllEmails = () => {
    const url = APIData.api + `avinya/organization-hierarchy?org=${org}`;
    axios
      .get(url, { headers: APIData.headers })
  ...
  const handlesubmit = () => {
    ...
    const url = APIData.api + `avinya/organization-hierarchy/update-supervisor/delete/id?emailId=${selectedPersonEmail}&newSupervisorEmail=${selectedAnotherEmail}`;
    const requestData = {
  ...
  const handlesubmit2 = () => {
    ...
    const url = APIData.api + 'avinya/organization-hierarchy';
    const requestData = {
              ...
              <TreeNode
                label={
                  <StyledNode>
                    ...
                    <img src={org.imageUrl} alt="" height={80} width={80} />
                    <p>{org.name}</p>
                    <p>{org.designation}</p>
                  </StyledNode>
                }
                key={org.email}
              >
                <Avhirearchy supervisorEmail={org.email} />
              </TreeNode>
            ))}
        </>
      )}
