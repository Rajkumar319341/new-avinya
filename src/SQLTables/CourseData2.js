...
if(sessiondetails){
  ...
  {   
      return ( <div>
     <Link to="/superaddcourses"><div className="Upload"> 
    ...
    >
      <Fields>
        ...
        <Field name="course_fees" label="Course Fees" placeholder="Course Fees" type="number" />
  ...
  {    
      return (<div> <Link to="/superaddcourses"><div className="Upload"> 
     {<AiIcons.AiFillPlusCircle />}
      </div></Link> 
    <CRUDTable
      caption="COURSES"
...
const service = {
  ...
  update: data => {
    ...
    axios.post(APIData.api+'courses/', data, {headers:APIData.headers})
      .then(response => {
        if(response.data.status.toString().toLowerCase() =="success"){
          toast(response.data.description)
