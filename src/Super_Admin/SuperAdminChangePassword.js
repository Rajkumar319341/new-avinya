import React,{Component}  from 'react';
import axios from 'axios';
import {toast} from 'react-toastify'; 
import 'react-toastify/dist/ReactToastify.css';
import {APIData} from '../Authentication/APIData';
import * as AiIcons from "react-icons/ai";
var passwordValidator = require('password-validator');
toast.configure()
var schema = new passwordValidator();
 
// Add properties to it
schema
.is().min(8)                                    // Minimum length 8
.is().max(32)                                  // Maximum length 100
.has().uppercase(1)                              // Must have uppercase letters
.has().lowercase(1)                              // Must have lowercase letters
.has().digits(1)    
.has().symbols(1)                            // Must have at least 2 digits
.has().not().spaces()                           // Should not have spaces
.is().not().oneOf(['Passw0rd', 'Password123']); // Blacklist these values
const sessiondetails = JSON.parse(localStorage.getItem("sessiondetails"));
class SuperAdminChangePassword extends Component {
  constructor(props) {
    super(props)
    this.state = {
      username: sessiondetails.user,
      password: null,
      hidden: true
    }
    this.handlePasswordChange = this.handlePasswordChange.bind(this);
    this.toggleShow = this.toggleShow.bind(this);

  }

  handlePasswordChange(e) {
    this.setState({ password: e.target.value });
  }

  toggleShow() {
    
    this.setState({ hidden: !this.state.hidden });

  }


      changeHandler = e => {
        this.setState({ [e.target.name]: e.target.value })
      }

      submitHandler = e => {
        e.preventDefault()
        if (schema.validate(this.state.password)) {
          axios
          .post(APIData.api+'login-type/update', this.state,{headers:APIData.headers})
          .then(response => {
              if(response.status===200){
                toast("Password Changed")
                //change URL LATER
                const url = new URL(APIData.url);
                window.location.href = url;
                localStorage.clear();
              }
              else
              {
                toast("Password Update Failed")
              }
            
                
            
        })
          .catch(error => {
            toast("Its time to grab coffee")
            console.log(error)
          })  
        } else { 
          toast("Enter a strong Password That Includes Atleast 1 Lowercase, 1 Uppercase, 1 Numeric & 1 Special Character & The Length Must Be Atleast 8 Characters or Longer")
        }


      }
    render() { 
        return (  
            
            <div className="superAdminChangePassword">
                <form className="signin">
        <div className="carrybox">
            <div className="card-header">
            <h3 className="heading01">Change Password</h3>
            <div  className="bringitcenter">
            <img className="profile_img" src="data:image/png;base64," alt="Profile" />
            </div >
            <div 
            className="bringitcenter">
                <p className="ptag">ID :
                <strong className="ptag">{sessiondetails.user}
                </strong>
                </p>
                </div>
            </div>         
      <div className="row">
      <div className="carrybox">
      <label 
      className="heading02" 
      htmlFor="Confirmpassword">Enter New Password :</label>
      
      <div className="Passwordfield">
      <input 
                  className="enter_field" 
                  type={this.state.hidden ? "password" : "text"}
      id="Confirmpassword" 
      name="password" 
      value={this.state.password}
      onChange={this.changeHandler} 
      onClick={this.handlePasswordChange}
      placeholder="Confirm Password" />
<div className="holdeye" onClick={this.toggleShow}>
<div className="EyeMark" ><div className="Eyecenter"><AiIcons.AiFillEye/></div></div>
</div>
                  {this.state.password_error ? (<div style={{color:'red' }}>
                  {this.state.password_error}
                  </div>)  : null}

      </div>
      </div>
      </div>

      <div className="row">
            <button className="Submitbutton" type="submit" onClick = {this.submitHandler}>Update</button>
      </div>
            </div>  
           
      </form>
            </div>
        );
    }
}
 
export default SuperAdminChangePassword;