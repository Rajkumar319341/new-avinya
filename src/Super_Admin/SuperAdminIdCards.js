...
export class SuperAdminIdCards extends Component {
    state = {
        selectedID: "",
        display: false,
        dob: "",
    ...
    changeHandler = (e) => {
        ...
        axios.get(url, { headers: APIData.headers })
            .then((response) => {
                ...
                console.log('Res name :', response.data.faculty_id)


                console.log("Res photo url:", response.data.photoUrl)
                console.log('Name from Api:', response.data)
                this.setState({ name: response.data.name, dob: response.data.dob, phone: response.data.phone_number, id: response.data.faculty_id, photo: response.data.photoUrl, display: false })
            })
    }
...
export default SuperAdminIdCards