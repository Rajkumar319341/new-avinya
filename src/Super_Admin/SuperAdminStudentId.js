...
export class SuperAdminStudentId extends Component {
    state = {
        selectedID: "",
        display: false,
        dob: "",
    ...
    changeHandler = (e) => {
        ...
        const url = APIData.api + "students/" + e.target.value;
        console.log(url)
        axios.get(url, { headers: APIData.headers })
            .then((response) => {
                console.log('Res', response)
                console.log('Name from Api:', response.data)
                this.setState({ name: response.data.name, dob: response.data.dob, phone: response.data.phone_number, id: response.data.student_id, photo: response.data.photo,display: false })
            })
    }
...
export default SuperAdminStudentId